...
rule gwas_single_chrom:
	...
	params:
		...
		maf = config["maf"]
	output:
		out = "output/{run_name}/gwas/{run_name}.{phenotype}.chr{chr}.850K.mlma",
		log = "output/{run_name}/gwas/{run_name}.{phenotype}.chr{chr}.850K.log"
	shell:
		# "code/gcta_1.93.2beta/gcta64 --bfile {params.iprefix} --mlma --pheno {input.phenotypes} --mpheno {params.phenotype} --grm {params.grmprefix} --maf {params.maf} --autosome-num 29 --thread-num {params.threads} --out {params.oprefix}"
		"""
		psrecord "code/gcta_1.93.2beta/gcta64 --bfile {params.iprefix} --chr {params.chrom} --mlma --pheno {input.phenotypes} --mpheno {params.phenotype} --grm {params.grmprefix} --maf {params.maf} --autosome-num 29 --thread-num {params.threads} --out {params.oprefix}" --log {params.psrecord} --include-children --interval 30
		"""
